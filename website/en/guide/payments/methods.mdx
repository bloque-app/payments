...
Accept cash payments at authorized locations. When the customer chooses cash, the payment result includes a `payment_code` (10-digit).
...
## Filter Payment Methods (Hosted Checkout UI)
...
| `cash` | Cash payment (returns a payment code) |
...
1. User selects cash payment in hosted checkout
2. The checkout returns a `payment_code`
3. Customer completes payment at an authorized location
4. Your webhook receives `payment.status.updated` when the status changes
